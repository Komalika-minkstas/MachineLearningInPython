...
print('Accuracy of GaussianNB on training set :{:.2f} '.format(nbclf.score(X_train, y_train)))
print('Accuracy of GaussianNB on test set :{:.2f} '.format(nbclf.score(X_test, y_test)))
...
print('Random Forest, default settings, Fruit Dataset')
...
print('Random Forest, Cancer Dataset')
print('Accuracy of RF classifier on training set : {:.2f}'.format(clf.score(X_train, y_train)))
print('Accuracy of RF classifier on test set : {:.2f}'.format(clf.score(X_test, y_test)))
...
title='GBDT, default settings, complex binary dataset'
...
#Gradient Boosted Decision Tree on real world dataset
X_train, X_test, y_train, y_test=train_test_split(X_cancer, y_cancer, random_state=0)

clf=GradientBoostingClassifier().fit(X_train, y_train)
print('Breast Cancer Dataset')
print('Accuracy of GBDT, default settings on training set: {:.2f}'.format(clf.score(X_train, y_train)))
print('Accuracy of GBDT, default settings on test set: {:.2f}'.format(clf.score(X_test, y_test)))
clf=GradientBoostingClassifier(learning_rate=0.01, max_depth=2, random_state=0).fit(X_train, y_train)
print('Breast Cancer Dataset')
print('Accuracy of GBDT settings on training set: {:.2f}'.format(clf.score(X_train, y_train)))
print('Accuracy of GBDT on test set: {:.2f}'.format(clf.score(X_test, y_test)))
...
plt.legend()
plt.title('Neural Network Functions')
...
title='Dataset 1: Neural Net Classifier, 2 layer, {} \ [10/10] units'
...
nnclf=MLPClassifier(hidden_layer_sizes=[100,100], alpha=5.0, random_state=0, solver='lbfgs').fit(X_train_scaled, y_train)
print('Breast Cancer Dataset')
print('Accuracy of NN classifier on training dataset:{:.2f}'.format(clf.score(X_train_scaled, y_train)))
print('Accuracy of NN classifier on test dataset:{:.2f}'.format(clf.score(X_test_scaled, y_test)))


# In[24]:

#The effect of different choices of activation
X_train, X_test, y_train, y_test=train_test_split(X_D2, y_D2, random_state=0)
fig, subaxes=plt.subplots(3,1,figsize=(6,18))
...
for thisaxisrow, thisactivation in zip(subaxes, ['tanh','relu']):
    for thisalpha, thisaxis in zip([0.0001,1.0,100], thisaxisrow):
        ...
        thisaxis.set_ylabel('Target feature')
        thisaxis.set_title('MLP Regressor\nalpha={}, activation={}'.format(thisalpha, thisactivation))
        plt.tight_layout()
